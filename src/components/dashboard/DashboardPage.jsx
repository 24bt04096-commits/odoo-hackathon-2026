...
export const DashboardPage = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8 relative font-sans selection:bg-brand-500 selection:text-white">
      ...
      {/* ========================================================= */}
      {/* 1.5 BUDGET HIGHLIGHTS SUMMARY WIDGET                      */}
      {/* ========================================================= */}
      {(() => {
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-soft space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-base font-extrabold text-slate-900">Budget Highlights</h3>
                  <p className="text-xs text-slate-500">Overall financial summary across all planned itineraries</p>
                </div>
              </div>
              <button
                onClick={() => setCurrentScreen('budget')}
                className="text-xs font-bold text-brand-600 hover:text-brand-700 bg-brand-50 hover:bg-brand-100 px-3.5 py-1.5 rounded-xl transition-colors"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-200/60">
                ...
                <p className="text-xl font-black text-slate-900 mt-0.5">{formatCurrency(totalBudget)}</p>
              </div>

              <div className="bg-emerald-50/50 rounded-xl p-3.5 border border-emerald-100">
                <span className="text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Total Spent</span>
                <p className="text-xl font-black text-emerald-700 mt-0.5">{formatCurrency(totalSpent)}</p>
              </div>

              <div className="bg-brand-50/50 rounded-xl p-3.5 border border-brand-100">
                <span className="text-[11px] font-bold text-brand-600 uppercase tracking-wider">Remaining Balance</span>
                <p className="text-xl font-black text-brand-700 mt-0.5">{formatCurrency(remaining)}</p>
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              <div className="flex justify-between text-xs font-bold text-slate-600">
                <span>Budget Spent Progress ({percentSpent}%)</span>
                <span>{formatCurrency(totalSpent)} of {formatCurrency(totalBudget)}</span>
              </div>
              <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200/60">
                <div 
                  className={`h-full rounded-full transition-all duration-500 ${
                    percentSpent > 90 ? 'bg-rose-500' : percentSpent > 70 ? 'bg-amber-500' : 'bg-brand-500'
                  }`}
